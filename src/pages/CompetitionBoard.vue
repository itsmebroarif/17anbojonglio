<template>
  <div class="min-h-screen bg-slate-100 text-slate-800 p-6 sm:p-10 flex flex-col justify-between font-sans relative overflow-hidden select-none">
    <!-- Top Header Bar -->
    <div class="flex items-center justify-between border-b border-slate-300 pb-6">
      <div class="flex items-center space-x-4">
        <div class="w-12 h-12 rounded-2xl bg-red-600 flex items-center justify-center font-extrabold text-xl text-white shadow-md shadow-red-500/30">
          17
        </div>
        <div>
          <h1 class="text-2xl sm:text-3xl font-extrabold tracking-wider uppercase text-slate-900">
            {{ store.settings.eventName }}
          </h1>
          <p class="text-xs text-red-600 font-mono tracking-widest uppercase">
            LIVE ARENA COMPETITION BOARD • {{ currentTime }}
          </p>
        </div>
      </div>

      <!-- Select Active Competition for Board & Print Preview Button -->
      <div class="no-print flex flex-wrap items-center gap-3">
        <button
          @click="isPrintPreviewOpen = true"
          class="px-3.5 py-2 bg-red-600 hover:bg-red-700 text-white font-bold text-xs rounded-xl shadow-md transition-colors flex items-center gap-1.5"
        >
        ...
        <select
          ...
          class="bg-white border border-slate-300 text-slate-900 font-bold px-4 py-2 rounded-xl text-sm focus:outline-none focus:border-red-500 shadow-xs"
        >
        ...
        <router-link
          to="/"
          class="p-2 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 rounded-xl text-sm border border-slate-300 shadow-xs"
        >
    ...
    <div v-if="activeComp" class="my-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        <div class="bg-white p-8 rounded-3xl border-2 border-red-500/40 shadow-xl relative overflow-hidden">
          ...
          <span class="text-xs font-bold text-red-600 uppercase tracking-widest block mb-2">
          ...
          <div class="mt-8">
            <div v-if="playingParticipants.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div
                ...
                :key="p.reg.id"
                class="bg-red-50/50 border-2 border-red-500 p-6 rounded-2xl flex items-center space-x-4 shadow-sm"
              >
                ...
                <div>
                  <h3 class="text-xl font-bold text-slate-900 leading-tight">{{ p.part?.name }}</h3>
                  <p class="text-xs text-slate-600 mt-1">Umur {{ p.part?.age }} Thn • {{ p.part?.address || 'Warga RT/RW' }}</p>
                </div>
              </div>
            </div>

            <div v-else class="py-12 text-center text-slate-500 border-2 border-dashed border-slate-300 rounded-2xl bg-slate-50">
              <i class="bi bi-display text-4xl block mb-2 text-slate-400"></i>
              <p class="text-sm font-medium">Belum ada peserta yang berstatus <strong>PLAYING</strong>.</p>
              <p class="text-xs text-slate-500 mt-1">Ubah status peserta di Waiting Lounge untuk menampilkan arena live.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-md flex flex-col justify-between">
        <div>
          <div class="flex items-center justify-between mb-4 border-b border-slate-200 pb-3">
            <h3 class="font-extrabold text-base uppercase text-amber-600 flex items-center gap-2">
          ...
          <div class="space-y-3 max-h-[380px] overflow-y-auto pr-1">
            ...
            >
              <div class="flex items-center space-x-3">
                <span class="font-mono font-bold text-sm bg-amber-100 text-amber-800 px-2.5 py-1 rounded-lg border border-amber-200">
                  {{ p.reg.participantNumber }}
                </span>
                <span class="font-bold text-sm text-slate-800">{{ p.part?.name }}</span>
              </div>
              <span
                class="text-[10px] font-bold px-2 py-0.5 rounded uppercase"
                :class="p.reg.status === 'Called' ? 'bg-blue-100 text-blue-800 border border-blue-200' : 'bg-slate-200 text-slate-700'"
              >
                {{ p.reg.status }}
              </span>
            </div>

            <div v-if="nextQueueParticipants.length === 0" class="py-8 text-center text-slate-400 text-xs">
    ...
    <div v-else class="my-20 text-center py-16 bg-white rounded-3xl border border-slate-200 shadow-sm max-w-xl mx-auto">
      <i class="bi bi-display text-6xl text-slate-300 block mb-3"></i>
      <h2 class="text-xl font-bold text-slate-800">Silakan Pilih Cabang Lomba</h2>
    ...
    <!-- Bottom Ticker Bar -->
    <div class="border-t border-slate-300 pt-4 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-2">
